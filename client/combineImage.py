...
def combineImage(x, y, w, h, folderName, i) :
    # #--① 이미지 읽기
    ...
    content = cv2.imread('./static/images/' + folderName + '/changeBackground' + i + '.png')
    
    img[y:y+h, x:x+w] = content # 원본 이미지에서 선택 영영만 ROI로 지정 ---⑥
    cv2.imwrite('./static/images/' + folderName + '/conbine' + i + '.png', img)   # ROI 영역만 파일로 저장 ---⑦
